...
const getItemTotal = (item: any) => {
  ...
  const returnablePrice = item.product.returnable ? (parseFloat(item.product.returnable_price) || 0) : 0
  const quantity = Number(item.quantity) || 0
...
const addToCart = (product: any) => {
  ...
  if (existingItem) {
    if (existingItem.quantity < (product.actual_stock || product.stock_quantity)) {
...
const handleCheckout = async () => {
  ...
  try {
    ...
    if (result.success) {
      // Se foi venda a prazo, atualizar crédito usado do cliente
